    ...
    <div className="p-4 sm:p-6 md:p-8 h-full">
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-5 sm:mb-6">
        {[
          ...
          <div key={label} className={cn('rounded-2xl px-3 py-3 sm:p-4 border min-w-0', bg, border)}>
            <p className={cn('text-xl sm:text-3xl font-semibold tracking-tight', color)}>{count}</p>
            <p className="text-[11px] sm:text-sm text-gray-500 mt-0.5 leading-tight break-words">{label}</p>
          </div>
        ))}
      </div>

      {/* Filter tabs */}
      <div className="mb-5 -mx-4 px-4 pb-2 sm:mx-0 sm:px-0 sm:pb-0 overflow-x-auto">
      ...
      {loading ? (
        ...
        <div className={cn('flex flex-col xl:flex-row gap-4 sm:gap-5', selected ? 'items-start' : '')}>
          ...
          {selected && (
            <>
              <div className="hidden xl:block w-full flex-1 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div className="p-4 sm:p-6 space-y-5">
                  ...
                  <div>
                    ...
                    {selected.screenshot_url ? (
                      <div className="rounded-2xl border border-gray-200 bg-gradient-to-b from-gray-50 to-white p-3 sm:p-4">
                        <div
                          className="relative rounded-xl overflow-hidden border border-gray-200 cursor-zoom-in group bg-white"
                        ...
                        >
                          ...
                          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                            <div className="opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity bg-white/95 rounded-lg px-3 py-1.5 flex items-center gap-1.5 text-xs font-medium text-gray-700 shadow-sm">
                              <ExternalLink className="w-3.5 h-3.5" /> View fullscreen
                            </div>
                          </div>
                        </div>
                        <p className="text-[11px] text-gray-400 mt-2">
                          Tap the preview to open the full screenshot.
                        </p>
                      </div>
                    ) : (
              ...
              <div className="xl:hidden fixed inset-0 z-[120] bg-black/40 backdrop-blur-sm p-3 flex items-end">
                <div className="w-full max-h-[90vh] overflow-y-auto bg-white rounded-t-3xl rounded-b-2xl shadow-2xl border border-gray-100">
                  <div className="sticky top-0 z-10 bg-white/95 backdrop-blur px-4 py-3 border-b border-gray-100 rounded-t-3xl">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="text-sm font-semibold text-charcoal mt-2 line-clamp-2">{selected.description}</p>
                      </div>
                      <button
                        onClick={() => setSelectedId(null)}
                        className="px-3 py-1.5 text-xs font-medium text-gray-500 border border-gray-200 rounded-lg"
                      >
                        Close
                      </button>
                    </div>
                  ...
                  <div className="p-4 space-y-5">
                        ...
                        <div className="rounded-2xl border border-gray-200 bg-gradient-to-b from-gray-50 to-white p-3">
                          ...
                          >
                            ...
                            <div className="absolute inset-0 bg-black/0 transition-colors flex items-center justify-center">
                              <div className="bg-white/95 rounded-lg px-3 py-1.5 flex items-center gap-1.5 text-xs font-medium text-gray-700 shadow-sm">
                                <ExternalLink className="w-3.5 h-3.5" /> View fullscreen
                              </div>
                            </div>
                          </div>
                    ...
                    <button
                      onClick={() => deleteReport(selected.id)}
                      className="w-full flex items-center justify-center gap-1.5 px-3 py-2.5 text-xs font-medium text-red-500 hover:bg-red-50 rounded-xl transition-colors border border-red-100"
                    >
